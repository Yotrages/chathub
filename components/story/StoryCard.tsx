...
const StoryCard: React.FC<EnhancedReelCardProps> = ({
  ...
  const containerClasses = isFullscreen
    ...
    : isCompact
    ? "w-24 xs:w-32 h-48 aspect-[9/16] bg-white dark:bg-gray-800 rounded-lg shadow-md dark:shadow-lg overflow-hidden"
    : "w-full h-full bg-white dark:bg-gray-800 rounded-lg shadow-md dark:shadow-lg overflow-hidden";
        ...
        {isTextOnlyStory ? (
          ...
          <div className="relative w-full h-full">
            ...
            {/* {story.fileType === "video" && !isCompact && (
              <button
                onClick={toggleMute}
                className="absolute top-4 hidden sm:flex right-4 bg-black bg-opacity-50 text-white rounded-full p-2 hover:bg-opacity-70 transition-all z-10"
              >
                {isMuted ? (

...
const ReelCard = forwardRef<HTMLDivElement | null, EnhancedReelCardProps>(({
  ...
  useEffect(() => {
    if (videoRef.current) {
      ...
      if (isFullscreen && ref) {
        (ref as React.MutableRefObject<HTMLDivElement>).current = videoRef.current.parentElement as HTMLDivElement;
      }
    }
  ...
  const handleShareToChat = async (chatId: string) => {
    try {
      ...
      await sendMessage(chatId, messageContent, "post", undefined, undefined, undefined, reel._id);
  ...
  const groupedReactions = reel.reactions?.reduce((acc, reaction) => {
  ...
  return (
    <div ref={ref} className={containerClasses}>
      <div className={`relative ${isCompact ? "w-32 h-48" : "w-full h-full"}`}>
        <video
          ref={videoRef}
          src={reel.fileUrl}
          ...
          <>
            ...
            <div className="absolute right-4 bottom-20 flex flex-col space-y-4 z-10">
              <button
                ...
                onMouseOver={() => window.screen.availWidth > 768 && setShowEmojiPicker(true)}
              ...
              >
                {userReactionEmoji ? (
                  <>
                    <span className="text-2xl">{userReactionEmoji.category}</span>
                    <span className="text-white text-xs font-medium">{reel.reactions.length}</span>
                  </>
                ) : (
                  <>
      ...
      {showComments && (
        <div onClick={handleBackdropClick} className="fixed inset-0 z-50 bg-black bg-opacity-75 flex items-end">
                ...
                >
                  {reel.reactions && reel.reactions.length > 0 && (
                    <div className="flex -space-x-1">
                      {Object.entries(groupedReactions).slice(0, 3).map(([emoji]) => (
                        <span key={emoji} className="text-lg bg-white rounded-full border-2 border-white">
                          {emoji}
                        </span>
                      ))}
                    </div>
